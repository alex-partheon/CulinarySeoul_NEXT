...
import { TeamSwitcherProvider } from '@/components/team-switcher';
...
const MetricCardsSection = React.memo<{ stats: CompanyStats }>(function MetricCardsSection({
  stats,
}) {
  return (
...
export default function CompanyDashboard() {
  ...
  React.useEffect(() => {
    ...
    if (!hasAnyRole(['super_admin', 'company_admin'])) {
      router.push(
        '/auth/signin?error=unauthorized&message=회사 대시보드에 접근할 권한이 없습니다.',
      );
      return;
    }
        ...
        <SidebarProvider>
          <div className="flex h-screen w-full overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <main className="flex-1 overflow-y-auto bg-gradient-to-br from-slate-50 via-white to-slate-50">
                <CompanyAdminUp
                  fallback={<AccessDenied message="회사 대시보드에 접근할 권한이 없습니다." />}
                >
